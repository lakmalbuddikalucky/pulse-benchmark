...
class MetricsTracker:
    ...
    def compute_overall_metrics(self) -> Dict[str, Any]:
        ...
        if self.results["predictions"] and self.results["labels"]:
            ...
            # Store only the metrics we're tracking and convert numpy types to Python types for JSON serialization
            overall_summary = {}
            for metric in self.metrics_to_track:
                if metric in overall_metrics:
                    ...
                    if isinstance(value, (np.integer, np.floating, np.ndarray)):
                        if np.isnan(value):
                            overall_summary[metric] = None
                        else:
                            overall_summary[metric] = float(value)
                    else:
                        overall_summary[metric] = value
...
def calculate_pulse_score(
    y_true: Union[np.ndarray, torch.Tensor],
    y_pred: Union[np.ndarray, torch.Tensor],
    y_pred_binary: Union[np.ndarray, torch.Tensor] = None,
    threshold: float = 0.5,
) -> float:
    """
    Calculate PULSE score using the PULSEScoreCalculator.

    Args:
        y_true: Ground truth labels (0 or 1)
        y_pred: Predicted probabilities (0..1)
        y_pred_binary: Binary predictions (optional, will be derived from y_pred if not provided)
        threshold: Threshold to convert probabilities to binary predictions

    Returns:
        PULSE score (0-100 scale)
    """
    try:
        ...
        if y_pred_binary is None:
            y_pred_binary = (y_pred >= threshold).astype(int)
        elif isinstance(y_pred_binary, torch.Tensor):
            y_pred_binary = y_pred_binary.detach().cpu().numpy()

        # Initialize PULSE calculator
        pulse_calculator = PULSEScoreCalculator()

        # Calculate PULSE score for single outcome
        pulse_result = pulse_calculator.calculate_pulse_score_single_outcome(
            y_true=y_true, y_prob=y_pred, y_pred=y_pred_binary
        )

        return pulse_result["pulse_score"]

    except ImportError:
        logger.warning(
            "PULSEScoreCalculator not available. Returning NaN for PULSE score."
        )
        return np.nan
    except Exception as e:
        logger.warning(f"Could not calculate PULSE score: {e}. Returning NaN.")
        return np.nan
...
def calculate_all_metrics(
    ...
    metrics = {
        ...
        "pulse_score": calculate_pulse_score(y_true, y_pred, threshold=threshold),
